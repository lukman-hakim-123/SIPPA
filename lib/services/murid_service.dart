...
class MuridService {
  ...
  MuridService({required TablesDB db, required Storage storage})
    : _db = db,
      _storage = storage;

  Future<Result<User>> createMurid(User user) async {
    try {
  ...
  Future<Result<User>> getMuridById(String userId) async {
    try {
  ...
  Future<Result<User>> updateMurid(User user) async {
    try {
  ...
  Future<Result<List<User>>> getAllMuridBySekolah(String sekolah) async {
    try {
      ...
      final muridList = documents.rows.map((doc) {
        final Map<String, dynamic> data = Map<String, dynamic>.from(doc.data);
        return User.fromMap(data);
      }).toList();
      return Result.success(muridList);
    } catch (e) {
      return Result.failed(e.toString());
    }
  }

  Future<Result<List<User>>> getAllMuridByKelompok(
    String sekolah,
    kelompok,
  ) async {
    try {
  ...
  Future<Result<List<User>>> getAllMurid() async {
    try {
      final documents = await _db.listRows(
        databaseId: dotenv.env['APPWRITE_DATABASE_ID']!,
        tableId: dotenv.env['APPWRITE_USERS_COLLECTION_ID']!,
        queries: [Query.equal('levelUser', 3), Query.orderDesc('\$createdAt')],
      );
      final muridList = documents.rows.map((doc) {
        final Map<String, dynamic> data = Map<String, dynamic>.from(doc.data);
        return User.fromMap(data);
  ...
  Future<Result<void>> deleteMurid(String muridId) async {
    try {
      await _db.deleteRow(
        databaseId: dotenv.env['APPWRITE_DATABASE_ID']!,
        tableId: dotenv.env['APPWRITE_USERS_COLLECTION_ID']!,
